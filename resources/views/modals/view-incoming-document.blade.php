    ...
    <div class="px-5 py-8">
        ...
        <hr class="bg-gray-400">
            ...
            <div>
                ...
                <template x-if="comments != 'N/A'"><div>Request Comments: <span class="font-bold underline" x-text="comments"></span></div></template>
                <template x-if="rejection_reason != 'N/A'"><div>Rejection Reason: <span class="font-bold underline" x-text="rejection_reason"></span></div></template>
            </div>

            <!--Buttons-->
            <div class = "justify-between w-full inline-flex">
                <div class = "pt-10 gap-3 justify-between inline-flex">
                    <button x-on:click.prevent="$dispatch('open-modal', {name: 'incoming-document-preview', trackingCode: tracking_code, filePath: file_path})"
                ...
                <div>
                    ...
                    <div class = "gap-3 inline-flex">
                        <button x-on:click.prevent="$dispatch('open-modal', {name: 'reject-request', trackingCode: tracking_code, requestId: request_id})" x-bind:disabled="!(requested || is_forwarded)" :class="(requested || is_forwarded) ? 'bg-red-600 hover:bg-red-800 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150' : 'bg-gray-300 cursor-not-allowed'"
                            class = "inline-flex items-center p-4 border border-transparent rounded-md text-white  tracking-widest">Reject</button>
                        <button x-on:click.prevent="$dispatch('open-modal', {name: 'accept-request', trackingCode: tracking_code, requestId: request_id})" x-bind:disabled="!(requested || is_forwarded)" :class="(requested || is_forwarded) ? 'bg-green-600 hover:bg-green-900 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150' : 'bg-gray-300 cursor-not-allowed'"
...
<x-modal name="document-preview" :maxWidth="'2xl'">
    <div x-data="previewData" @open-modal.window="if($event.detail.name === 'incoming-document-preview'){
